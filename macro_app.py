...
pydirectinput.PAUSE = 0.01  # Adjust this value if needed
...
class MacroGUI(QMainWindow):
    ...
    def play_macro(self):
        if self.is_playing:
            self.logger.warning("Macro is already playing")
            return

        self.is_playing = True
        self.pause_playback = False
        self.pause_button.setEnabled(True)
        self.logger.info("Starting macro playback")

        self.countdown_timer(3)  # Start the countdown

    def countdown_timer(self, seconds):
        self.countdown_seconds = seconds
        self.countdown_label.show()  # Show the countdown label
        self.countdown_timer = QTimer(self)
        self.countdown_timer.timeout.connect(self.update_countdown)
        self.countdown_timer.start(1000)  # Update every second
        self.update_countdown()  # Call immediately to show initial count

    def update_countdown(self):
        if self.countdown_seconds > 0:
            self.countdown_label.setText(f"Macro starts in: {self.countdown_seconds}")
            self.countdown_seconds -= 1
        else:
            self.countdown_timer.stop()
            self.countdown_label.hide()  # Hide the countdown label
            self.execute_macro()

    def execute_macro(self):
        self.statusBar().showMessage("Macro is running...")
        try:
            for index, action in enumerate(self.macro_actions):
                while self.pause_playback:
                    time.sleep(0.1)  # Wait while paused
                    if not self.is_playing:
                        return  # Stop playback if is_playing became False

                self.logger.debug(f"Executing action {index + 1}: {action}")
                time.sleep(action['delay'] / self.playback_speed)
                
                if action['type'] == 'keydown':
                    try:
                        self.logger.debug(f"Pressing key: {action['key']}")
                        pydirectinput.keyDown(action['key'])
                    except Exception as e:
                        self.logger.error(f"Error during key press: {e}")
                        QMessageBox.warning(self, "Playback Error", f"Error during key press: {e}")
                        break
                elif action['type'] == 'keyup':
                    try:
                        self.logger.debug(f"Releasing key: {action['key']}")
                        pydirectinput.keyUp(action['key'])
                    except Exception as e:
                        self.logger.error(f"Error during key release: {e}")
                        QMessageBox.warning(self, "Playback Error", f"Error during key release: {e}")
                        break
                elif action['type'] == 'click':
                    try:
                        self.logger.debug(f"Clicking at ({action['x']}, {action['y']}) with button {action['button']}")
                        pydirectinput.click(action['x'], action['y'], button=action['button'])
                    except Exception as e:
                        self.logger.error(f"Error during click: {e}")
                        QMessageBox.warning(self, "Playback Error", f"Error during click: {e}")
                        break

                if not self.is_playing:
                    break  # Stop playback if is_playing became False
        
        except Exception as e:
            self.logger.error(f"Unexpected error during macro playback: {e}")
            QMessageBox.critical(self, "Playback Error", f"An unexpected error occurred: {e}")
        
        finally:
            self.logger.info("Macro playback completed or stopped")
            self.is_playing = False
            self.pause_button.setEnabled(False)
            self.pause_button.setText("Pause")
            self.statusBar().showMessage("Macro playback finished", 3000)  # Show for 3 seconds
    ...
    def type_text(self, text):
        for char in text:
            if char.isupper():
                ...
                pydirectinput.press(char.lower())  # Press the lowercase version of the character
                pydirectinput.keyUp('shift')
            else:
                pydirectinput.press(char)  # Simulate pressing the character
            time.sleep(0.05)  # Optional: Add a small delay between key presses
    ...
    def input_text(self):
        ...
        if ok and text:
            ...
            for char in text:
                action = {'type': 'keydown', 'key': char, 'delay': 0.1}  # Adjust delay as needed
                self.macro_actions.append(action)
                self.macro_list.addItem(f"Key Down: {char}, Delay: 0.10s")  # Update the macro list
                action = {'type': 'keyup', 'key': char, 'delay': 0.1}  # Key up action
                self.macro_actions.append(action)
                self.macro_list.addItem(f"Key Up: {char}, Delay: 0.10s")  # Update the macro list
